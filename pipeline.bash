...
#/Applications/bowtie2/bowtie2-build --threads $threads $genome_dir $genome_ref
##################################################################################################################################################################################################
...
#	/Applications/bowtie2/bowtie2 -p $threads -q --local -x $genome_ref -U $fastq_in/$sample_name".fastq.gz" -S $align_out/$sample_name.sam &> $align_out/$sample_name.log

	# SAM to BAM conversion using samtools
#	/Applications/samtools-1.14/samtools view -h -S -b -@ $threads -o $bam_out/$sample_name.bam $align_out/$sample_name.sam
...
#	sambamba sort -t $threads -o $sorted_bam/$sample_name"_sorted.bam" $bam_out/$sample_name.bam
	...
	#/Applications/samtools-1.14/samtools index -@ $threads $filtered_sorted_bam/$sample_name"_sorted_filtered.bam"
